...
import os
from AgenticGraph import AgenticGraph, StartNode, EndNode, MessageDict
    ...
    for product in state.products:
        ...
        if existing_analysis:
            ...
            response = client.completion(
                prompt=f"{anthropic.HUMAN_PROMPT} {system_prompt}\n\nPrevious context:\n{context}\n\nAnalyze the product: {product}{anthropic.AI_PROMPT}",
...
def marketing_strategist(graph_data: Dict[str, Any], agentic_memory: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for analysis in state.analyses:
        ...
        if existing_strategy:
            ...
            response = client.completion(
                prompt=f"{anthropic.HUMAN_PROMPT} {system_prompt}\n\nPrevious context:\n{context}\n\nCreate a marketing strategy based on this analysis: {analysis.analysis}{anthropic.AI_PROMPT}",
                max_tokens_to_sample=300,
                model="claude-2"
            )
            strategy = response.completion
    ...
    for analysis, strategy in zip(state.analyses, state.strategies):
        if analysis.market_size == "Small" and "niche" in analysis.target_audience.lower():
            ...
            if existing_refined_strategy:
                ...
                response = client.completion(
                    prompt=f"{anthropic.HUMAN_PROMPT} {system_prompt}\n\nPrevious context:\n{context}\n\nRefine this strategy for a niche market:\nProduct: {analysis.product_name}\nAnalysis: {analysis.analysis}\nCurrent Strategy: {strategy.strategy}{anthropic.AI_PROMPT}",
                    max_tokens_to_sample=300,
                    model="claude-2"
                )
                refined_strategy = response.completion
